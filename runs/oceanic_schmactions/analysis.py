...
class cluster_animator(object):
    def __init__(self, snapshots, xaxis='x', yaxis='y',
        ...
        if self.plot_panel:
            ...
            self.ax_traj_vert.set_xlabel('r [kpc]')
            ...
            self.ax_traj_vert.plot(np.sqrt(x*x + y*y), z, c='k', alpha=0.5)
            ...
            self.traj_vert_current, = self.ax_traj_vert.plot(np.sqrt(x*x + y*y), z, c='k')
    ...
    def _animate_(self, frame, scat, im=None):
        ...
        if self.plot_panel:
            ...
            r = np.sqrt(x[frame]*x[frame] + y[frame]*y[frame])
            self.traj_vert_scat.set_offsets(np.transpose([r, z[frame]]))
            ...
            self.traj_vert_current.set_xdata(np.sqrt(x*x+y*y))
