...
for gal,ax_col in zip(glist, ax.transpose()):
    ...
    ax_col.set_ylim(-200, 200)

    fit = np.load('fit_'+gal+'.npy')

    ax_col.plot(theta/np.pi, (midplane_est-fit)*1000, c=tb_c[0])
    ax_col.plot(theta/np.pi, (err_low-fit)*1000, c=tb_c[0], ls='dashed', alpha=0.5)
    ax_col.plot(theta/np.pi, (err_high-fit)*1000, c=tb_c[0], ls='dashed', alpha=0.5)
    ax_col.fill_between(theta/np.pi, (err_high-fit)*1000, (err_low-fit)*1000, color=tb_c[0], alpha=0.25)
