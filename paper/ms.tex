...
collaboration\footnote{\url{https://fire.northwestern.edu}} \citep{2016ApJ...827L..23W,2017arXiv170206148H}.  In Section \ref{sssec:neighborhood} we use these estimates to consider, as an example, the feasibility of one science goal that relies on the invariance of actions --- the reconstruction of open clusters across large distances using ``dynamical tagging;'' that is, the association of stars to a birth cluster based on their similar actions. In light of our findings, we considered the orbits of the
...
%The actions are computed using the technique described in Section~\ref{ssec:action_comp}. 
 In Section~\ref{sssec:reconstruction} we estimate and discuss the size of the dynamical solar neighborhood for each cluster based on its mass, to show that action-space tagging is likely still a viable method if background is unimportant. In Section~\ref{sec:discussion} we discuss further implications for action-space analyses beyond the dynamical solar neighborhood. We summarize and conclude in Section~\ref{sec:conclusion}.
