...
have the attractive quality of being invariant and thus good quantitative labels
of a star's orbit. A systematic bias in the phase space position
induces 
%mm a phase-dependence [to better distinguish the two different
%meanings of phase in this sentence!]
   an orbital phase dependence
in the actions, which we interpret as a systematic error
distinct from typical observational errors and Galactic potential uncertainties.
An offset in the vertical position of 
%mm
     the Galactic midplane of
$\sim15\,\pc$ 
%mm [this formulation isn't worth the extra difficulty reading]
%($\sim120\,\pc$) for a thin (thick) disk 
  for a thin disk orbit or $\sim 120\,\pc$ for a thick disk
orbit is sufficient to induce a $25\%$ systematic error in the
vertical action $J_z$. The induced error distribution is non-Gaussian and
bimodal, with neither of the modes peaking on the null value. Furthermore, we
...
to the Galactic center, the orientation of the Galactic 
%mm plane, the height of the Sun above the Galactic midplane, 
   midplane, the distance of the Sun from it,
...
%mm The main point of this paper is as follows. Due to relatively
%small-scale variations in the density of gas and stars, the local
%midplane should vary as a
...
%mm For stars relatively far from the Sun, their local midplane will
%differ 
...
%mm [the x-axis of this figure should be labeled orbital phase for
...
%mm phase-dependence of the actions calculated from a phase-space
%point and an
...
%mm ``true'' orbit of the star; that is, as it oscillates around the
%true midplane.
...
This example uses an offset in $z$, but one 
%mm would expect an analogous effect from an offset in another coordinate, 
...
\label{tab:orbits}} \tablehead{\colhead{name} & \colhead{initial position} &
\colhead{initial velocity} & \colhead{$J_R$} & \colhead{$J_{\phi}$} &
...
$\beta$ are 
%mm arbitrary phases
    the initial orbital phases at $t = 0$.
...
%mm an NFW halo \citep{1997ApJ...490..493N},
...
We now 
%mm describe the intuition behind 
...
of action values. We plot this quantity
%mm
     in Figure~\ref{fig:Jz_hist}, 
which we refer to as $\Delta J_i$ for each action, 
%mm in Figure~\ref{fig:Jz_hist} 
as a horizontal arrow anchored on the true action value. Because of the bimodality
...
%mm
     \kms$,
 respectively, 
%mm [one significant figure values]
      giving $J_z \simeq 0.7, 20,$ and $500\,\actunit$  
(see Table~\ref{tab:orbits}).} For a \thin{}-like orbit ($J_z\sim
%mm 0.5
         0.7%
\,\actunit$), 
even a
...
\section{
%mm Coordinate System Offsets from Azimuthal 
Midplane Variations}
...
%mm $\z = 0$ snapshots\footnote{In this work, to avoid
...
%mm As noted above, the  [not sure it has been?]
     The 
...
%mm [This looks like text, not footnote] \footnote{
...
%mm details of the simulation are given in
...
%mm
     is included.
%mm We use the L2 model, for which Sagittarius is
...
%mm $2.6\times10^4$, $1.2\times10^4$, 
       $2.6\times10^4\,\Msun$, $1.2\times10^4\,\Msun$, 
...
galaxies% 
%mm as a function of azimuth at the solar circle
. Starting from the coordinate system
described in the previous section, which is 
approximately 
%mm
    \MM{[isn't this by definition, at least on average, not
      approximate?]} 
aligned so that the
...
%mm [this was written backwards.  Reordered.] 
%We consider $50$ equally spaced bins in azimuth, a sufficient number
%such that each cylinder shares no stars with its neighboring
%cylinder. 
At 
%mm each value of 
...
%mm [redundant with text] The procedure is performed again using the new height $10$ times
%to converge on the local midplane height. 
...
%mm \text{A} \sin{\left(\phi + \text{B}\right)} + \text{C}\eeq curve
    curve of the form 
...
%\mi{}, \mf{}, and \mm{}, respectively. 
    ...
    (-165, 45, 8.8)\,\pc$, $B = (0.67, -0.088, 0.031)\,\text{rad}$,
    and $C = (-69, 19, -18)\,\pc$.
...
%mm [change to rad for consistency with B.   Note that you did not
%give two significant figures of accuracy for the last value.
%Strictly speaking should be corrected, though this is pretty minor.]
%$\text{A}$ --- we compute $1.15$, $0.31$, and $0.06\,\deg$ for \mi{},
%\mf{}, and \mm{}.  
      $A$. We find for the same models $\Delta \theta = (0.020, 5.4
      \times 10^{-3}, 1.0 \times 10^{-3})\,\text{rad}$.
...
%$185$, $162$, $84\,\pc$ for \mi{}, \mf{}, and \mm{}. 
...
%mm [again, you are missing significant digits in B: always report the
%same number. When you gave 3 I cut back to two, but I can't fill in when
%you only give one.] $\text{A} \sin{\left(\phi + \text{B}\right)} +
%\text{C}$, with the values for $\text{A}$ being $9.5$, $2.5$, $-212$, and
%$-394\,\pc$, for $\text{B}$ being $0.0013$, $0.0007$, $-0.63$, and
%$-1.00\,\text{rad}$ and $\text{C}$ being $2.6$, $-7.8$, $-65$, and
%$-53\,\pc$ chronologically for each of the four timesteps shown. 
    given in Equation~(\ref{eq:fit-curve}), with the values at times
    $(2, 4, 6, 6.9)\,\Gyr$ being $A = (9.5, 2.5, -2.1 \times 10^2,
    -3.9 \times 10^2)\,\pc$, $B
    = (1.3 \times 10^{-3}, 7 \times 10^{-4}, -0.63,
    -1.0)\,\text{rad}$, and $C = (2.6,-7.8, -65, -53)\,\pc$.
The middle $90\%$ of midplane values across the solar circle spans 
%mm $49$, $62$, $137$, and $119\,\pc$, for each timestep.
       $(49, 62, 1.4 \times 10^2, 1.2 \times 10^2)\,\pc$.
...
%mm high-star formation regions. 
...
approximation 
%mm (Equation~\eqref{eq:Ji_err_mosttime}, [eqref adds unneeded
%parentheses here] 
         (Eq.~\ref{eq:Ji_err_mosttime}),
...
pre-\textit{%mm [adding an EOL comment to eliminate spurious space]
...
%mm apply also to studies which
...
%mm (as defined by one half the middle 90th percentile). 
...
%mm issues raised in the last three points.
   large errors in actions summarized above.

\item 
%mm To demonstrate the severity of the previous point, we measured
...
\textit{Gaia} 
%mm DR2 \citep{2018A&A...616A..11G, 2019arXiv190209569F} 
...
potential. 
%mm [the "true" midplane is spatially and time dependent, so I'm not
%sure this makes sense as something that could be proposed.] 
%We do not propose here what a modeler should use for the ``true''
...
action diffusion,
%mm which motivated this paper in the first place, 
...
D'Onghia, Benoit Famaey, 
%mm and Adrian Price-Whelan
    Adrian Price-Whelan, and Todd Phillips
for helpful
discussions. 
%mm A.B. would like to thank Todd Phillips for helpful discussions. 
A.B. was supported
...
Environment (XSEDE), 
%mm
    \MM{[Does anyone want their XSEDE grant number here?]}
which is supported by 
%mm National Science Foundation 
     NSF
grant 
%mm number 
OCI-1053575. K.V.J.'s contributions were supported in part by the
%mm National Science Foundation 
     NSF
under grants 
%mm NSF 
...
%mm NSF 
AST-1614743.
M.-M.M.L. was partly supported by NSF grant 
%mm AST18-15461. [for consistency with above]
        AST-1815461. 
%mm The work of D.A.-A., A.B., D.W.H., R.E.S., K.V.J., M.-M.M.L., and
%M.K.N. is supported by the Simons Foundation. [This is redundant with
%our Flatiron Institute affiliations, unless there is additional,
%separate, support for any of us.]
