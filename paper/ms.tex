...
the local standard of rest (LSR). We review the observational efforts to measure these parameters in Section~\ref{sec:mes_err}.
...
simulation of a Sagittarius encounter with a galaxy otherwise tailored to the stellar mass, scale length, and scale height of the Milky Way \citep{2018MNRAS.481..286L}.
...
for action-space analyses beyond the dynamical solar neighborhood. In light of our findings, we considered the
...
from \citet{2018AA...616A..10G}. Actions are computed using the method outlined in Section~\ref{ssec:action_comp}, with typical values for the Galactocentric coordinate system parameters. These are only shown to illustrate the general types of orbits open clusters are on. The neighborhood column is explained in more
...
\colhead{$J_{\phi}$} & \colhead{$J_z$} & \colhead{$z_{\text{max}}$} & \colhead{mass} \\ 
\colhead{ } & \colhead{ } &
\colhead{$\mathrm{pc}$} & \colhead{$\mathrm{kpc\,km\,s^{-1}}$} &
\colhead{$\mathrm{kpc\,km\,s^{-1}}$} & \colhead{$\mathrm{kpc\,km\,s^{-1}}$} &
\colhead{$\mathrm{pc}$} & \colhead{$M_{\odot}$}}
\startdata
alphaPer & 740 & 170 & 20 & -1700 & 0.005 & 11 & 352\tablenotemark{b} \\
Blanco1 & 489 & 240 & 2 & -1800 & 2 & 210 & 410\tablenotemark{c} \\
ComaBer & 153 & 90 & 2 & -1900 & 0.7 & 140 & 112\tablenotemark{d} \\
Hyades & 515 & 50 & 20 & -1800 & 0.3 & 80 & 400\tablenotemark{e} \\
IC2391 & 325 & 150 & 6 & -1800 & 0.02 & 20 & \nodata \\
IC2602 & 492 & 150 & 10 & -1700 & 0.3 & 89 & \nodata \\
NGC2451A\tablenotemark{a} & 400 & 190 & 6 & -1800 & 0.2 & 73 & \nodata \\
Pleiades & 1326 & 140 & 20 & -1700 & 0.4 & 96 & 800\tablenotemark{f} \\
Praesepe & 938 & 190 & 20 & -1800 & 0.6 & 120 & 550\tablenotemark{d}
\enddata
...
frequencies, $\Omega_c \equiv v_c/R_g$ the rotation frequency, $A_z$ and $A_R$ are the amplitudes of the stellar motion in the
...
In this case, we have that for $J_{\phi}$ \citep[][Section~3.5.3b]{2008gady.book.....B}, 
...
coordinates, respectively. Since $a_R = -\kappa^2(R-R_g)$, and $a_z = - \nu^2
z$, we have that,
\beq\label{eq:JR_Jz_epi_energy}
\begin{split}
J_R &= \frac{\frac{1}{2}v_R^2 + \kappa^2 (R-R_g)^2}{\kappa} \\
J_z &= \frac{\frac{1}{2}v_z^2 + \nu^2 z^2}{\nu}
\end{split}
\eeq

While the formula for $J_{\phi}$ is phase independent, $J_R$ and $J_z$ are
both phase dependent via Equation~\eqref{eq:orbits_epi}. We consider two cases. First, at the point of maximum
amplitude or when $(v_R, v_z) = (0, 0)$ and second, at the point of maximum
velocity $(R, z) = (R_g, 0)$. For $J_R$ we have that,
\beq\label{eq:JR_epi_final}
    \begin{aligned}
        J_R &= \kappa A_R^2 && \text{if $v_R = 0$} \\
    J_R &= \frac{v_{R,\text{max}}^2}{2\kappa^2} && \text{if $R=R_g$} \text{.}
    \end{aligned}
\eeq
For $J_z$, we have that,
\beq\label{eq:Jz_epi_final}
    \begin{aligned}
        J_z &= \kappa A_z^2 && \text{if $v_z = 0$} \\
        J_z &= \frac{v_{z,\text{max}}^2}{2\kappa^2} && \text{if $z=0$} \text{.}
    \end{aligned}
\eeq

Now assume that the coordinates $(R, z, v_{\phi}, v_R, v_z)$ are offset by
$(\Delta R, \Delta z, \Delta v_{\phi}, \Delta v_R, \Delta v_z)$. Then, the
induced error in $J_{\phi}$ is, regardless of phase,
\beq\label{eq:induced_Jphi}
\frac{\Delta J_{\phi}}{J_{\phi}} = \frac{\Delta R}{R_g}\text{.}
\eeq
For $J_R$, the induced error is,
\beq\label{eq:induced_JR}
\begin{aligned}
    \frac{\Delta J_R}{J_R} &= \frac{2\Delta R}{A_R} + 
        \left(\frac{\Delta v_R}{v_{R,\text{max}}}\right)^2 && \text{if $v_R = 0$} \\
    \frac{\Delta J_R}{J_R} &= \frac{2\Delta v_R}{v_{R,\text{max}}} + 
        \left(\frac{\Delta R}{A_R}\right)^2  && \text{if $R=R_g$} \text{.}
\end{aligned}
\eeq
For $J_{z}$, the induced error is,
\beq\label{eq:induced_Jz}
\begin{aligned}
    \frac{\Delta J_z}{J_z} &= \frac{2\Delta z}{A_z} + 
        \left(\frac{\Delta v_z}{v_{z,\text{max}}}\right)^2 && \text{if $v_z = 0$} \\
    \frac{\Delta J_z}{J_z} &= \frac{2\Delta v_z}{v_{z,\text{max}}} + 
        \left(\frac{\Delta z}{A_z}\right)^2  && \text{if $z=0$} \text{.}
\end{aligned}
\eeq

Since most of the time stars will be at maximum amplitude (i.e. turnaround) in both $R$ and $z$, we can approximate the order of magnitude of the systematic error in the actions by
...
In the remainder of this section, we compare our analytic estimates of the effect on actions of a midplane offset against numerical calculations. A numerical evaluation of the effect of velocity offsets on actions is deferred to future work, as we discuss in Section~\ref{ssec:lsr_var}.
...
approximation from Equation~\eqref{eq:Ji_err_mosttime}. This Equation is relevant since during most of the orbit the star will be at maximum radial and vertical amplitude and we do not consider velocity offsets. Note that we consider an error
...
(Equations~\eqref{eq:actions_epi_error}~and~\eqref{eq:Jr_epi_dx}). In the
...
different local midplanes (see Figures~\ref{fig:midplane}~and~\ref{fig:midplane_chervin}). Since our criterion for cluster membership (distance in action space) scales with the
...
Equation~\eqref{eq:action_disp}. We then consult
...
Equation~\eqref{eq:action_disp} and convert the dispersion to a $z$ offset using
...
system induces phase-dependence in the actions calculated from the observed positions and velocities of stars
(Figures~\ref{fig:cartoon}~and~\ref{fig:one_orbit_wrong_ref}). Since stars' instantaneous phase-space positions are measured without prior knowledge of their orbital phases, this results in systematic error in the computed actions
(Figure~\ref{fig:many_orbit_wrong_ref}).

\item Inaccuracy in the midplane location most severely affects computation of the vertical action $J_z$.  A midplane offset of $\sim6\,\pc$ ($\sim50\,\pc$) for a typical thin (thick) disk orbit results in a  $10\%$ error in $J_z$ (as defined by one half the middle
$90\uth$ percentile). The fractional error is significantly less for halo orbits.

\item The distribution of systematic errors in the actions induced by a
coordinate system offset is very non-Gaussian. The distribution is bimodal with
{\em neither mode at null}. 
As a result, error propagation of coordinate system offsets is complex when considering actions, and is likely to significantly deform the action-space distribution function.

\item Dynamical modelling across large regions of the disk, over which the midplane location varies by more than the limits discussed above, is
susceptible to this type of systematic error, since the assumption that our local
...
that the midplane varies as a function of azimuth at the Solar circle by $\sim185, 162, 84\,\pc$ (middle $90\%$) for the
...
\item While in this work we have focused on systematic errors in action computation, all of our conclusions also extend to studies of stars that simply rely on orbit
...
the Galaxy, and that this variation can lead to significant systematic errors in the computation of actions under the assumption of a global axisymmetric potential. We do
...
azimuthal dependence of the midplane location. For some applications, such as those using actions as labels to group stars on similar orbits, using such a measurement to shift stars to a consistent midplane height as a function of azimuth before using a global axisymmetric approximation to the potential may be sufficient, although this ignores the \emph{dynamical} implications of shifts in the midplane height (which result from fluctuations in the local density). However, for other applications, such as the study of action diffusion which motivated this paper in the first place, a more extensive perturbative approach is likely to be needed. We plan to explore the mitigation of these effects in future work.

\acknowledgments
We would like to thank Megan Bedell, Robert A. Benjamin, Tobias Buck, Elena D'Onghia, Benoit
...
\implies \Delta R &\equiv \abs{R_{\text{err}} - R} = \abs{\cos{\phi}} \Delta x\text{.}
\end{split}
\eeq
