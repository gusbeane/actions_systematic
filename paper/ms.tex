...
the distribution of observed $J_z$ values. Similar plots for $J_R$ induced by
a $z$ offset and $J_R$ and $J_{\phi}$ induced by an $x$ offset are given in
...
from a live N-body simulation of a Sagittarius-like encounter. This simulation offers us the
...
$6\times10^{10}\,\Msun$, and a Hernquist bulge of mass $10^{10}\,\Msun$ and scale
length $0.7\,\kpc$ \citep{1990ApJ...356..359H}. We use the L2 model, for which Sagittarius is modeled as a
dark matter Hernquist sphere of mass $8\times10^{10}\,\Msun$ and scale length
$8\,\kpc$, along with a stellar component modeled as a Hernquist sphere
...
The fiducial coordinate system for these N-body simulations is the rest frame of
the aligned host galaxy at the beginning of the simulation.
...
\end{center}
\caption{The local midplane determined at the fiducial Solar radius
($8.2\,\kpc$) for 4 different time steps from live N-body simulations
...
but for four succcessive timesteps of the live N-body simulation of a Sagittarius encounter
...
\end{center}
\caption{A histogram of the observed values in $J_{\phi}$ for the thick-disk (top
panel) and thin-disk (bottom panel) orbits assuming a $z$ offset of
$100\,\pc$. The induced error distribution is non-Gaussian and bimodal, with
neither of the modes centered on null.}
