...
\title{Issues with Dynamical Modelling Across the Disk}
...
We propose a new type of error in the computation of stellar actions. This
error is induced by the fact that the local Galactic midplane varies across
the disk. As a result, when we make 6D phase space measurements of stars that
are non-local (i.e. far away), we are observing those stars with a systematic
offset in $z$. We show that this systematic offset induces time evolution in
the observed actions which we interpret as an error. We note that this error
is non-Gaussian and bimodal, with neither of the modes peaking on the true
action value. A midplane error of $\sim30\,\pc$ and $\sim225\,\pc$ is
...
of the midplane must be taken into account when performing dynamic modelling
