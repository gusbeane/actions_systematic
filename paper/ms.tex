...
the same as the actions.\footnote{This ignores any potential error in the epicyclic and vertical frequencies.} The energy in the azimuthal coordinate can be readily
computed ($E_{\phi} = \frac{1}{2}v_{\phi}^2$). Using standard propagation of errors, we see that $\Delta E_{\phi}/E_{\phi}$ and $\Delta J_{\phi}/J_{\phi}$ (Equation~\eqref{eq:induced_Jphi}) differ by a $\Delta R/R_g$ term. Since $R_g\approx 8\,\kpc$, we expect this term to be negligible and we therefore conclude that:
